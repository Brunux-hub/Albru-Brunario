#!/usr/bin/env pwsh
# Script para probar la persistencia del seguimiento al recargar la pantalla
...
Write-Host "  PRUEBA DE PERSISTENCIA DEL SEGUIMIENTO" -ForegroundColor White
Write-Host "═══════════════════════════════════════════════════════════════════════" -ForegroundColor Cyan
Write-Host ""
...
# PASO 2: Verificar que el cliente aparece en la lista del asesor
Write-Host "📋 PASO 2: Verificando cliente en lista del asesor..." -ForegroundColor Yellow
Write-Host ""

Start-Sleep -Seconds 2

try {
    $clientes = Invoke-RestMethod -Uri "http://localhost:3001/api/clientes/asesor/$AsesorId"
    
    if ($clientes.success) {
        $clienteEncontrado = $clientes.clientes | Where-Object { $_.id -eq $ClienteId }
        
        if ($clienteEncontrado) {
            Write-Host "   ✅ Cliente encontrado en la lista del asesor" -ForegroundColor Green
            ...
            Write-Host "   🔓 Opened At: $($clienteEncontrado.opened_at)" -ForegroundColor Cyan
            Write-Host "   👨‍💼 Asesor Asignado: $($clienteEncontrado.asesor_asignado)" -ForegroundColor Cyan
        } else {
            Write-Host "   ❌ Cliente NO encontrado en la lista del asesor" -ForegroundColor Red
            exit 1
        }
    } else {
...
Write-Host "📋 PASO 3: Simulando recarga de pantalla (nueva consulta)..." -ForegroundColor Yellow
Write-Host ""

Start-Sleep -Seconds 1

try {
    $clientes2 = Invoke-RestMethod -Uri "http://localhost:3001/api/clientes/asesor/$AsesorId"
    
    if ($clientes2.success) {
        $clienteEncontrado2 = $clientes2.clientes | Where-Object { $_.id -eq $ClienteId }
        
        if ($clienteEncontrado2) {
            Write-Host "   ✅ Cliente SIGUE apareciendo después de recarga" -ForegroundColor Green
            ...
            if ($clienteEncontrado2.seguimiento_status) {
                Write-Host ""
                Write-Host "   ✅ SEGUIMIENTO_STATUS SE MANTIENE: $($clienteEncontrado2.seguimiento_status)" -ForegroundColor Green -BackgroundColor Black
                Write-Host ""
            } else {
                Write-Host ""
                Write-Host "   ❌ SEGUIMIENTO_STATUS ES NULL - PROBLEMA ENCONTRADO" -ForegroundColor Red -BackgroundColor Black
                Write-Host ""
                exit 1
            }
        } else {
...
Write-Host "  ✅ PRUEBA EXITOSA - SEGUIMIENTO SE MANTIENE AL RECARGAR" -ForegroundColor White -BackgroundColor DarkGreen
Write-Host "═══════════════════════════════════════════════════════════════════════" -ForegroundColor Green
Write-Host ""
